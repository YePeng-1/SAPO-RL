...
import numpy as np
from ADMM_Sbeta import ADMM_Sbeta
import numpy as np
import pandas as pd
...
test_files_numpy = [["SolutionInputDP54.npy","SolutionInputDP48.npy"],
...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="SequentialFuselageActuators",
    ...
    return args
args = parse_args()
def make_env(env_id, seed, idx, capture_video, run_name, n_actions, mode, end_mode, record):
    def thunk():
        env = gym.make(env_id, env_no=idx, n_actuators=n_actions, mode=mode, end_mode=end_mode, record=record, seed=seed)
...
class Run_Ansys:
    ...
    def show_deformation(self):
        ...
        self.result.plot_nodal_displacement(0, show_displacement=True, displacement_factor=10000)
        return
...
class TestData:
    """Helper class to simulate the Y structure from MATLAB"""
    def __init__(self, SN, MN):
        # Generate random test data
        np.random.seed(42)
        self.Ytest_YZ = np.random.randn(SN, MN) * 0.1  # Small random values

def _get_errors(deviations):
    # Needs to be called after getting observations so that data is up to date
    # Calculate error relative to perfect circle with r=288
    n = len(deviations)
    dev_total = np.sqrt(np.square(deviations[:177]) + np.square(deviations[177:]))
    max_e = max(dev_total) # maximum error
    mae = sum(np.abs(deviations))/n # mean absolute error
    rmse = np.sqrt(sum((deviations)**2)/n) # root mean squared error
    mse = sum((deviations)**2)/n # mean squared error
    se = sum((deviations)**2) # sum of squared errors
    return rmse, max_e
...
surrogate_mdl = load(path.join(path.dirname(__file__), 'AssemblyGym/envs/FuselageActuators/Surrogates', 'surrogate_likeDu_v22.joblib') )
# read U matrix
...
for i in range(30):
    folder = path.join(path.dirname(__file__), 'AssemblyGym/envs/FuselageActuators/Shapes', 'Test')
    file1 = test_files_numpy[i][0]
    filepath = path.join(folder, file1)
    initPos = np.load(filepath)

    folder = path.join(path.dirname(__file__), 'AssemblyGym/envs/FuselageActuators/Shapes', 'Test')
    file2 = test_files_numpy[i][1]
    ...
    deviations = _get_deviation(initPos, displacements, targetPos)
    initDev = deviations


    # Initialize the error
    error, maxDev = _get_errors(deviations)
    error_initial = error
    maxDev_initial = np.max(np.abs(initDev))

    mu1_max = 1
    mu1_min = 0

    #LN = mu1_max / np.max(np.abs(2*np.dot(U_mtx.transpose(), intercept + initDev)))
    LN = mu1_max / np.max(np.abs(U_mtx))
    U_mtx_mag = LN * surrogate_mdl.coef_
    intercept_mag = LN * surrogate_mdl.intercept_
    initDev_mag = LN * initDev
    ...
    while True:
        ...
        # Call ADMM solver
        betaE, history = ADMM_Sbeta(mu1, U_mtx_mag, intercept_mag+initDev_mag)

        # Find non-zero indices
        qindex = np.where(betaE != 0)[0]

        if len(qindex) == args.n_actions:
            count = 1
            break
        ...
        if len(qindex) < args.n_actions:
            mu1_max = mu1
        elif len(qindex) > args.n_actions:
            mu1_min = mu1
    # Find final non-zero indices
    qindex = np.where(betaE != 0)[0]
    n_act = len(qindex)

    displacements = (np.dot(U_mtx, betaE.reshape(-1, 1)) + intercept.reshape(-1, 1)).reshape(-1, 2)
    deviations = _get_deviation(initPos, displacements, targetPos)
    error, maxDev = _get_errors(deviations)
    forces = betaE.flatten()

    info = {"initError": error_initial, "Error": error, "Forces": forces, "maxDev": maxDev,
            "initMaxDev": maxDev_initial, 'ActuatorNum': n_act}
    ...
    writer.add_scalar(f"charts/test_ActuatorNum", info["ActuatorNum"], i)

    ansys_info = ansys_run.run(forces, i)
    ...
    writer.add_scalar(f"charts/Ansys_ActuatorNum", ansys_info["ActuatorNum"], i)
    ...
    if i < 4:
        Show_Deviations_Max(forces, actuatorIds, targetPosVis, initPosVis, finalPosVis, forcesActive,
                            forcesInactive,
                            ansys_info["initMaxDev"], ansys_info["maxDev"], mag)
        Show_Deviations_Error(forces, actuatorIds, targetPosVis, initPosVis, finalPosVis, forcesActive,
                              forcesInactive,

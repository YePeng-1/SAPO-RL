...
def Show_Deviations_Max(forces_,actuatorIds_,targetPosVis_,initPosVis_,finalPosVis_,forcesActive_,forcesInactive_,init_Max_Deviation_,final_Max_Deviation_,mag_):
    ...
    axs[1].quiver(finalPosVis_[actuatorIds_[forcesActive_],0], finalPosVis_[actuatorIds_[forcesActive_],1], forces_Y_[forcesActive_], forces_Z_[forcesActive_], angles='xy', scale=15, units='xy')
...
class Agent_Test():
    def __init__(self, args,model_):
        super().__init__()
        self.device = args.device
        self.policy_net = model_
        return
    ...
    def select_action_test(self, obs):
        ...
        if sample > eps_threshold:
            with torch.no_grad():
                scale = 1 / torch.max(torch.abs(self.policy_net(obs)))
                q_val = (F.sigmoid(scale * self.policy_net(obs)) + 1)
                return torch.mul(sel_obs, q_val).argmax(1).reshape(-1, 1)
        else:
            random_q_val = torch.rand(sel_obs.shape).to(self.device)
            return torch.mul(sel_obs, random_q_val).argmax(1).reshape(-1, 1)
...
class Dueling_DQN(nn.Module):
    ...
    def __init__(self, action_dim, space_dim, hidden_dim=128,dropout_rate=0.5):
        super(Dueling_DQN, self).__init__()

        self.action_dim =action_dim
        self.hidden_dim = hidden_dim

        self.fc0 = torch.nn.Linear(2 * space_dim, hidden_dim)
        self.bn1_0 = nn.BatchNorm1d(hidden_dim)
        self.dropout0 = nn.Dropout(p=dropout_rate)

        self.fc1 = torch.nn.Linear(hidden_dim, hidden_dim)
        self.bn1_1 = nn.BatchNorm1d(hidden_dim)
        self.dropout1 = nn.Dropout(p=dropout_rate)

        self.fc2 = torch.nn.Linear(hidden_dim, hidden_dim)
        self.bn1_2 = nn.BatchNorm1d(hidden_dim)
        ...
        self.fc5 = torch.nn.Linear(action_dim, 1)

        # self.maxpool = torch.nn.MaxPool1d(kernel_size=action_dim)

    def forward(self, x):
        gap_cpl = x[:, -1, :-1].unsqueeze(1)
        U_cpl = x[:, :-1, :-1]
        gap_cpl_exp = gap_cpl.expand_as(U_cpl)
        U_g_CPL = torch.cat((U_cpl, gap_cpl_exp), 2)
        ...
        y = F.leaky_relu(y)
        # y = self.dropout2(y)
        y = y.view(-1, self.action_dim, self.hidden_dim)

        adv = F.leaky_relu(self.fc3(y)).squeeze(2)

        val = F.leaky_relu(self.fc4(y)).squeeze(2)

        val = F.leaky_relu(self.fc5(val))
...
policy_net = Dueling_DQN(18, 354, hidden_dim=128).to(device)

state_dict = torch.load('./Agents/SequentialFuselageActuators__D3QN_SequentialFuselageActuators__1__1744185882.pth',map_location=device)
policy_net.eval()
policy_net.load_state_dict(state_dict)


agent = Agent_Test(args,policy_net)
...
for i in range(30):
    final_test_env = gym.make(args.env_id, env_no=i, n_actuators=args.n_actions, mode="Surrogate_Test", end_mode='fix_num', record=False, seed=args.seed)
    test_next_obs_np, info = final_test_env.reset()
    test_next_obs = torch.Tensor(test_next_obs_np).to(device).unsqueeze(0)
    while 1:
        with torch.no_grad():
            test_action = agent.select_action_test(test_next_obs)
        test_next_obs, test_reward, test_done, _, info = final_test_env.step(test_action.squeeze(0).cpu().numpy())
        test_next_obs = torch.Tensor(test_next_obs).to(device).unsqueeze(0)
        if test_done:
            break
